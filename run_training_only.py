#!/usr/bin/env python3
"""
Wrapper script to run training only from the root alert-fatigue directory.
"""
...
def main():
    """Run the training only script from the model_pipeline directory."""
    ...
    script_path = "run_training_only.py"
    if not os.path.exists(script_path):
        print(f"ERROR: run_training_only.py not found in {model_pipeline_dir}")
        return
    
    print(f"Running training only from: {os.getcwd()}")
    print("=" * 50)
